...
class FakeAnalysis:
    def __init__(self, id: int, accession: str, *, amplicon: bool = True, have_ann: bool = True, results_dir: Optional[str] = "."):
        self.id = id
        ...
        self.experiment_type = (
            self.ExperimentTypes.AMPLICON if amplicon else "NOT_AMP"
        )
...
class FakeQuery:
    ...
    def filter_by_statuses(self, statuses):
        kept = [a for a in self._items if all(a.status.get(st, False) for st in statuses)]
        return FakeQuery(kept)
...
def fake_objects():
    ...
    items = [
        FakeAnalysis(2, "MGYA00000002", amplicon=True, have_ann=True, results_dir="/tmp/r2"),
        FakeAnalysis(1, "MGYA00000001", amplicon=True, have_ann=True, results_dir="/tmp/r1"),
        FakeAnalysis(3, "MGYA00000003", amplicon=False, have_ann=True, results_dir="/tmp/r3"),
        FakeAnalysis(4, "MGYA00000004", amplicon=True, have_ann=False, results_dir="/tmp/r4"),
...
def patch_analysis_objects(monkeypatch, fake_objects):
    ...
    monkeypatch.setattr(analyses.models.Analysis, "objects", fake_objects, raising=False)
...
def patch_import_fn(monkeypatch):
    ...
    def _fake_import_fn(analysis, dir_for_analysis: Path, allow_non_exist: bool = False):
        calls["count"] += 1
        calls["args"].append((analysis.accession, str(dir_for_analysis), allow_non_exist))
...
def test_flow_processes_only_valid_amplicon_analyses_and_logs(monkeypatch, patch_analysis_objects, patch_import_fn):
    # Run without max_count to process all valid items (2 will be processed successfully; 3 others skipped)
    logged = run_flow_and_capture_logs(flow_module.import_primer_identification_for_imported_analyses)
    ...
    assert "[MGYA00000005] skipped/failed: results_dir is not set for this analysis" in logs
