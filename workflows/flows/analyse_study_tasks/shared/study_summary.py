...
from analyses.models import Study, AssemblyAnalysisBatch
...
@flow
def generate_assembly_batch_summary(
    assembly_batch_id: uuid.UUID,
) -> Union[List[Path], None]:
    """
    Generate a study summary file for a single assembly analysis batch.

    Assembly batches have a different structure than samplesheet-based runs:
    - Results are in batch.get_pipeline_workspace(PipelineType.ASA)
    - completed_assemblies.csv is in the ASA workspace, not batch root
    - Summaries are written to study.results_dir with batch-specific prefix
    ...
    asa_workspace = assembly_batch.get_pipeline_workspace(
        AssemblyAnalysisBatch.PipelineType.ASA
    )

    # Validate ASA workspace exists
    results_dir = Directory(
        path=asa_workspace,
        rules=[DirectoryExistsRule],
    )

    # Find completed assemblies CSV in ASA workspace
    completed_assemblies_csv = (
        asa_workspace / EMG_CONFIG.assembly_analysis_pipeline.completed_assemblies_csv
    )
    results_dir.files.append(
        File(
            path=completed_assemblies_csv,
            rules=[FileExistsRule, FileIsNotEmptyRule],
        )
    )

    logger.info(f"Using completed assemblies from {completed_assemblies_csv}")
    ...
    if not study.results_dir:
        ...
        logger.info(f"Setting {study}'s results_dir to default {study.results_dir}")
        study.results_dir.mkdir(exist_ok=True, parents=True)
        study.save()
    ...
    # Get assembly-specific summary generator
    assembly_summary_generator = STUDY_SUMMARY_GENERATORS["assembly"]

    # Use batch ID prefix to distinguish between batches
    batch_prefix = f"{str(assembly_batch.id)[:8]}"
    ...
    with chdir(study.results_dir):
        ...
        with click.Context(assembly_summary_generator.summarise_analyses) as ctx:
            ctx.invoke(
                assembly_summary_generator.summarise_analyses,
                output_prefix=batch_prefix,
                assemblies=completed_assemblies_csv,
                study_dir=asa_workspace,
                outdir=study.results_dir,
            )

    generated_files = list(study_dir.path.glob(f"{batch_prefix}*_{STUDY_SUMMARY_TSV}"))
...
def merge_assembly_study_summaries(
    ...
    if isinstance(study, str):
        ...
        # Refresh from DB to get latest results_dir set by batch runs
        study.refresh_from_db()
