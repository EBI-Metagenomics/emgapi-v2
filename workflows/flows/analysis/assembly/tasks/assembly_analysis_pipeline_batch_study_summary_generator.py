...
def generate_assembly_analysis_pipeline_batch_summary(
    ...
    logger.info("Removing any previously generated study-level summaries")

    # This is required to avoid smashing any real batch summaries with the partially generated ones, or just old ones
    # Remove study-level TSV files (e.g., ERP104177_*_summary.tsv) but keep batch-level ones (UUID prefix)
    study_accession = study.accession
    study_summary_pattern = f"{study_accession}_*_summary.tsv"

    try:
        for old_summary in study_dir.path.glob(study_summary_pattern):
            logger.info(f"Removing old study summary file: {old_summary}")
            old_summary.unlink()
    except Exception as e:
        logger.warning(
            f"Failed to remove some old study summary files: {e}. Continuing anyway."
        )
    ...
    with chdir(study.results_dir):
        ...
        with click.Context(study_summary_generator.summarise_analyses) as ctx:
            ctx.invoke(
                ...
                # This file only contains the analyses the user selected, as the input samplesheet is
                # dynamically generated by the batch execution
                assemblies=Path(
