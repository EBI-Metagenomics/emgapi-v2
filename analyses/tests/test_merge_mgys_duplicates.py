...
def setup_duplicate_studies():
    ...
    assembly_public = Assembly.objects.create(
        assembly_study=MGnifyStudy.objects.get(id=10003),
        ena_study=ena_dup_a
    )
...
def test_print_duplicate_mgys_studies(setup_duplicate_studies, caplog):
    ...
    with caplog.at_level(logging.INFO):
        ...
        assert f"ENA accession {dup_studies['ena_dup_a'].accession} is linked to multiple MGnify Studies:" in caplog.text
        assert f"ENA accession {dup_studies['ena_dup_b'].accession} is linked to multiple MGnify Studies:" in caplog.text
        assert f"ENA accession {dup_studies['ena_single']} is linked to multiple MGnify Studies:" not in caplog.text
...
def test_reassign_runs_and_assemblies(setup_duplicate_studies, caplog):
    ...
    with caplog.at_level(logging.INFO):
        ...
        assert f"Moving 0 assemblies from MGYS00010001 to MGYS00006001" in caplog.text
        assert f"Moving 1 runs from MGYS00010001 to MGYS00006001" in caplog.text
        # private: run and assembly in same study
        assert f"Moving 1 assemblies from MGYS00010002 to MGYS00006002" in caplog.text
        assert f"Moving 1 runs from MGYS00010002 to MGYS00006002" in caplog.text
...
def test_clashing_runs_gives_warning(setup_duplicate_studies, caplog):
    ...
    with caplog.at_level(logging.INFO):
        ...
        assert f"DUPLICATE RUNS FOUND IN BOTH STUDIES: old MGYS00006001 and new MGYS00010001. No further action performed." in caplog.text
